export default class Chat {
  constructor(url) {
    ...
    window.addEventListener("beforeunload", () => {
      this.ws.send(
        JSON.stringify({ type: "deleteUser", user: this.currentUser })
      );
    });
  }
  ...
  showChat() {
    if (!document.querySelector(".container")) {
      ...
      container.innerHTML = `
            ...
            <section class="chat">
             <div class="chat-content"></div>
                    ...
                    <input class="input chat-form-input" type="text" aria-label="Your message" name="message" placeholder="Write your message..." required>
                </form>
            </section>
            `;
      ...
      chatForm.addEventListener("submit", (e) => {
        ...
        const time = `${new Date().toLocaleDateString()} ${new Date()
          .toLocaleTimeString()
          .slice(0, -3)}`;
        this.ws.send(
          JSON.stringify({
